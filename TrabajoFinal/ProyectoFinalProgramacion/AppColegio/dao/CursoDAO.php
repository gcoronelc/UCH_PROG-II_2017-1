...
class CursoDAO {
    ...
    public function createCurso(CursoDTO $curso) {
        ...
        $descripcion_curso = $curso->getDescripcion_curso();
        $fregistro_curso = $curso->getFregistro_curso();
        $estado_curso = $curso->getEstado_curso();
        $col_Docente_id_docente = $curso->getCol_Docente_id_docente();
        ...
        try {
            ...
            if ($codCurso == null) {
                if ($nomCurso == null) {
                    ...
                    $statement->bindParam(":descripcion_curso", $descripcion_curso);
                    $statement->bindParam(":fregistro_curso", $fregistro_curso);
                    $statement->bindParam(":estado_curso", $estado_curso);
                    $statement->bindParam(":col_Docente_id_docente", $col_Docente_id_docente);
    ...
    public function getNewCodigo()
    {
        ...
        try {
            $conexion = new Conexion();
            $cn = $conexion->getConexion();
            $sql = "SELECT MAX(codigo_curso) as codigo FROM col_curso";
            $statement = $cn->prepare($sql);
            ...
            if ($statement->execute()) {
                if ($resultado = $statement->fetch()) {
                    ...
                    {
                        $cur_n = explode('-', $resultado['codigo']);
                        $codigo = $cur_n[0].'-'.($cur_n[1] + 1);
                    }
                }
                else
                    $codigo = 'CUR-1';
